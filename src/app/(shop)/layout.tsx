...
export default function ShopLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      {/* Header */}
      <header
        className={`${headerBg} ${isPaper ? 'relative z-40 overflow-hidden' : 'sticky top-0 z-40'}`}
      >
        {isPaper && (
          <img
            src="/images/brand/paper/paper-header-bg.png"
            alt=""
            className="absolute inset-x-0 -bottom-[30px] w-full h-auto min-h-full object-cover object-bottom pointer-events-none"
          />
        )}
