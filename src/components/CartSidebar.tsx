...
export default function CartSidebar({ top = '4rem' }: { top?: string }) {
  ...
  // Hide the sidebar on the cart and checkout pages (redundant there)
  const hideOnPath = pathname === '/account/cart' || pathname?.startsWith('/account/checkout');
  if (hideOnPath) return null;

  // Sidebar is for logged-in customers only
  ...
  return (
    ...
    >
      <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
        ...
        <Link href="/account/cart" className="text-xs text-blue-600 hover:underline">View full cart</Link>
      </div>
